    ...
    fn handle_packet(
        ...
        if let Err(conn_err) = result {
            ...
            self.state = match conn_err {
                ...
                ConnectionError::JlsAuthFailed(_) => {
                    unreachable!("JLSAuthFailed isn't generated by packet processing");
                }
                ConnectionError::JlsForwardError(_) => {
                    unreachable!("JlsForwardError isn't generated by packet processing");
                }
            };
        }
...
pub enum ConnectionError {
    ...
    /// JLS Authentication failed, return the upstream addr for forwarding
    #[error("JLS Authentication failed")]
    JlsAuthFailed(JlsAuthInner),

    /// Jls Forward error
    #[error("JLS Forward Error")]
    JlsForwardError(String),
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub struct JlsAuthInner {
